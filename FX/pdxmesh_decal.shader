...
PixelShader =
{
	Code
	[[
		float4 CalcDecal( float2 UV, float3 Bitangent, float3 WorldSpacePos, float4 Diffuse )
		{
	...
	MainCode PS_world
	{
		...
		[[
			...
			PDX_MAIN
			{
				...
				Diffuse = CalcDecal( DetailUV, Input.Bitangent, Input.WorldSpacePos, Diffuse );
	...
	MainCode PS_local
	{
		...
		[[
			PDX_MAIN
			{
				...
				Diffuse = CalcDecal( Input.UV0, Input.Bitangent, Input.WorldSpacePos, Diffuse );
